...
app.run(['$rootScope', '$state', '$stateParams', '$http', 'configHttp', 'pluginHttp', 'historyHttp', 'socket',
    function ($rootScope, $state, $stateParams, $http, configHttp, pluginHttp, historyHttp, socket) {
        ...
        $rootScope.deviceTypes = [
            {name: "PC", value: ""},
            {name: "Android", value: "Mozilla/5.0 (Linux; Android 4.1.1; Nexus 7 Build/JRO03D) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166  Safari/535.19"},
            ...
            switch (toState.name) {
                ...
                case 'configDetail.plugins':
                    sideNav.links = [{icon: 'icon-play', target: 'run', action: true, text: 'Run Configuration'},
                        ...
                        {
                            icon: 'icon-book',
                            target: 'history.filter({filter: $stateParams.configId})',
                            action: false,
                            text: 'Crawl History'
                        },
                        {
                            icon: 'icon-pencil',
                            target: 'configCopy({copyId: $stateParams.configId})',
                            action: false,
                            text: 'New Copy'
                        },
                ...
                case 'crawl.pluginOutput':
                    sideNav.links = [{
                        icon: 'icon-wrench',
                        target: 'configDetail.main({configId: configId})',
                        action: false,
                        text: 'Configuration'
                    }]
